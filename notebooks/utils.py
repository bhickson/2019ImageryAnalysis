...
def createNewTileIndex(newIndex_loc, raster_loc, crs="epsg:2868", targetw=5000, targeth=5000):
    if os.path.exists(newIndex_loc):
        print(f"Tile index {newIndex_loc} already exists")
        tiledf = gpd.read_file(newIndex_loc)
    else:
        print(f"Creating new tile index {newIndex_loc}...")

        subBoxes = generateTileGeometries(raster_loc, targetw=targetw, targeth=targeth)
        goodBoxes = []
        with rio.open(ortho30cmvrt_loc) as src:
            for i, subBox in enumerate(subBoxes):
                minx, miny, maxx, maxy = subBox.bounds
                ul = (minx, maxy)
                ll = (minx, miny)
                lr = (maxx, miny)
                ur = (maxx, maxy)
                vals = np.array([x[0] for x in src.sample(xy=[ul, ll, lr, ur], indexes=1)])
                if np.all(vals==src.nodata):
                    continue
                else:
                    goodBoxes.append(subBox)

        tiledf = gpd.GeoDataFrame(geometry=goodBoxes, crs="epsg:2868")
        tiledf["path"] = tiledf.geometry.apply(lambda g: f"W{int(g.bounds[0])}")
        tiledf["row"] = tiledf.geometry.apply(lambda g: f"N{int(g.bounds[3])}")
        tiledf.to_file(newIndex_loc, driver="GPKG")

    return tiledf


def generateTileGeometries(vrt, targetw=5000, targeth=5000, buffer=50):
    boxes = []
    with rio.open(vrt) as src:
        kwargs = src.profile
        bnds = src.bounds
...
def getOrthoWindow(ortho_file, geometry):
    try:
        with rio.open(ortho_file) as src:
            ...
            win = from_bounds(*geometry.bounds, transform=src.transform).round_offsets().round_lengths().round_shape()
...
def getRoadsDistance(path, row, geometry, windowShape, roadFileDir="R:/ProjectData/PAG2019/EPCExtent_30cm/RoadDistances"):
...
def buildVRT(directory, outfile, ftype="tif"):
    if not os.path.isdir(outfile):
        outfile = os.path.join(os.path.abspath(directory), outfile)
    if os.path.exists(outfile):
        return
    files = glob(os.path.abspath(directory) + f"/*.{ftype}")
    print(len(files))
    vrt = gdal.BuildVRT(os.path.abspath(outfile), files)
    del vrt
    print(f"Created {os.path.abspath(outfile)}")
...
def createHAGTileIndex(tile_index, dem_raster, ofile):
    if os.path.exists(ofile):
        return gpd.read_file(ofile)
    
    with rio.open(dem_raster) as dem_src:
        dem2015_box = box(*dem_src.bounds)

        hag_tileIndex = tile_index[tile_index.within(dem2015_box)].copy()
        hag_tileIndex["HAG"] = None

        for i, row in hag_tileIndex.copy().iterrows():
            ...
            if np.all(vals==src.nodata):
                ...
                hag =True
            hag_tileIndex.at[i, "HAG"] = hag

    hag_tileIndex = hag_tileIndex[hag_tileIndex.HAG==True]
    hag_tileIndex.to_file(ofile, driver="GPKG")
    
    return hag_tileIndex


def getSentinelBandFile(band_num, data_loc, suffix=""):
    files = glob(data_loc + f"/{band_num}*{suffix}.tif")
    if len(files) == 1:
        return files[0]
    elif len(files) > 1:
